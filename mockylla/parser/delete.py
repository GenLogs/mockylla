...
def handle_delete_from(delete_match, session, state, parameters=None):
    ...
    keyspace_name, table_name, table_info = get_table(table_name_full, session, state)
    ...
    rows_to_keep = [
        row for row in table_data if not check_row_conditions(row, parsed_conditions)
    ]
